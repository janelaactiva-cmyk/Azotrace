...
export default function SuccessPage() {
  ...
  useEffect(() => {
    ...
    const checkUser = async () => {
      ...
      if (user) {
        setUser(user);
        // Associar checkout ao utilizador
        await associateCheckout(user.id);
      }
    };
    checkUser();
  ...
  const associateCheckout = async (userId: string) => {
    try {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data?.user) {
        // Associar checkout ao utilizador
        await associateCheckout(data.user.id);
        setUser(data.user);
        setTimeout(() => {
      ...
      <div style={{
        ...
        <div style={{ textAlign: 'center', marginBottom: '24px' }}>
          ...
          <p style={{ color: '#6b7280', fontSize: '14px' }}>
            Cria a tua conta para aceder aos serviços
          </p>
        </div>
